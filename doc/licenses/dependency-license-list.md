...
- 当前代码（截至本文更新时间）：后端仅使用 Python 标准库；仓库内未引入任何第三方 Python 包或 npm 依赖。
...
| Pillow（可选，未来用于图片缩略图） | 图片缩略图生成/压缩 | Python 包（pip/内置） | 待定 | PIL / HPND（宽松） | 引入后需在 `NOTICE` 中登记并保留其许可证/声明（如有）。 |
